...
# def mix_embeddings(hiddenrepresentation, ndata, embeddings, proj):
#     '''Combine node-specific trainable embedding ``hiddenrepresentation`` with categorical inputs
#     (projected by ``embeddings``) and numeric inputs (projected by ``proj``).
#     '''
#     e = []
#     for key, value in ndata.items():
#         if value.dtype == torch.int64:
#             e.append(embeddings[key](value))
#         elif value.dtype == torch.float32:
#             e.append(proj[key](value))
#     return hiddenrepresentation + torch.stack(e, 0).sum(0)
...
class PinSageConv(nn.Module):
    ...
    def forward(self, h, nodeset, nb_nodes, nb_weights):
        ...
        h_nodeset = get_embeddings(h, nodeset)  # (n_nodes, inputfeatures)
